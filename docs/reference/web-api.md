...
| Endpoint | Input |
| --- | --- |
| `POST /lumber/api/split/text` | JSON containing `text` and `input_format` (`markdown` or `html`) plus split options. |
| `POST /lumber/api/split/file` | Multipart form with `file`, `input_format` (`auto`, `markdown`, `html`, or `docx`), plus split options. |
