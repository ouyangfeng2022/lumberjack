...
- `Chunk.body` never renders ancestor/own headings (they live in `ancestor_headings`/`own_heading`); headings merged from internal sections remain in the body, and the framework adapters' `chunk_content(chunk, heading_context=True)` prepends the rendered breadcrumb
...
- Exact `Exact*Splitter` classes fully recount rendered text at every budget decision without the tokenizer text cache (`use_tokenizer_cache = False`): every document's split starts from a zero cache in production and caches are never reused across documents. Identical texts within one split (repeated heading paths, bodies counted at both the call site and the draft builder, oversized-block texts and pieces counted inside `BlockSplitter`) are deduplicated by a per-split memo (`_memo_count`); `BlockSplitter` counts through the splitter's `_memoized_count` hook so piece counts land in the same memo. Doomed candidates are skipped via sound part-sum lower bounds (two tokens of slack per join, valid for BPE-style tokenizers): exact subtree fit checks prune before rendering a subtree candidate (`_subtree_body_token_bound` includes descendant headings), sibling merge attempts pre-reject through `_merge_bound_exceeds`, and record/`pack_parts` packing skips joins whose parts already exceed the budget — accepted candidates are always verified by full recounts. Exact subtree draft entries carry `body_token_count=0` (per-entry counts feed no budget decision). `ChunkFinalizer` reuses exact drafts' split-time counts when the default text stages leave the rendered body unchanged and otherwise performs the authoritative final recount after seasoning and planing. Default unprefixed splitters use a running additive estimate with a per-split `_count_once` memo (cache-enabled, which also serves the finalizer's authoritative recount). `RecordSplitter` counts exactly through the same cache-free per-split memo. Every pipeline run clears the tokenizer text cache before splitting.
