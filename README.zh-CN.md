...
共享 AST 使用格式中立的 `DocumentInline`、`DocumentBlock` 与
`SectionNode`，其中保存供 splitter 使用的规范化 Markdown-like 渲染文本。
Markdown/HTML 的 `DocumentAST.source` 保留原始文本；二进制格式可以留空或提供规范化文本。
块的渲染文本不保证与源码逐字节对应。精确定义见[公开契约](docs/reference/public-contract.md)。
...
- `kind` 会规范化为小写，并成为 `DocumentBlock.kind`。
...
| **Incremental Sibling** | `incremental-sibling` | 同 Sibling 的拓扑结构，使用累加估算路径。 |
...
默认情况下，每个分块渲染出的 `body` 都会以祖先标题面包屑开头，随后保留该分块自身的标题（例如一个 H2 leaf 会渲染为 `# Title\n\n## Section`）。`Chunk.headings` 只保存祖先标题。设置 `render_headings=False` 可以从 `body` 中省略祖先标题面包屑；分块自身标题和 chunk 内部的相对标题仍会渲染。所有 splitter 拓扑都按渲染结果预算：
...
| **Subtree** | 省略祖先标题面包屑；所选 subtree 自身标题和内部相对标题仍会渲染 | **按渲染口径预算**：subtree 是否适合合并的判断只计算最终会渲染的标题。 |
| **Sibling**（默认） | 省略祖先标题面包屑；chunk 自身标题和 chunk *内部*的相对标题（例如合并 chunk 中的兄弟 `###` 标题）仍会渲染 | **按渲染口径预算**：切分预算只会因为隐藏祖先标题而增长。有祖先标题的分块，其切分计划（分块数量、边界）可能不同于 `render_headings=True`。 |

> [!NOTE]
> 所有 splitter 拓扑都会在 draft 层保留标题 token，让合并算术保持自洽——当两个 chunk 合并后共享的祖先前缀发生变化，被挤出的标题 token 会回到 body 中，作为仍会渲染的自身标题或内部相对标题。切分预算判断与运行中的 `estimated_token_count` 都按渲染口径处理，因此估算值始终贴合实际渲染出的 `body`（由于 separator 近似，它与 `token_count` 可能相差一两个 token）。

```python
# 所有 splitter 拓扑都按渲染口径预算
...
- **提问与讨论**:在 Discussions 尚未启用期间，请通过 [issue](https://github.com/ouyangfeng2022/lumberjack/issues/new/choose) 提交。
