...
def _detect_format(text: str | bytes | Path, format: str) -> str:
    ...
    if isinstance(text, Path):
        ...
        return "docx" if ext == ".docx" else "markdown"
    ...
    *,
    ...
    Args:
        text: Document content as plain Markdown text, raw DOCX bytes,
            or a :class:`pathlib.Path` pointing to a ``.md`` or ``.docx`` file.
        format: Input format hint.  One of ``"auto"`` (the default),
            ``"markdown"``, or ``"docx"``.  When ``"auto"`` the format is
            inferred from the input type and/or file extension.
